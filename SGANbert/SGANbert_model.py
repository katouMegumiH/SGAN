...
class PPATbert_model(nn.Module):
    ...
    def forward(self, input_ids, attn_mask, pos_ids, event_mask, graph, distan, causal=None):
        ...
        if self.need_cls:
            ...
            node_embedding = torch.cat((node_embedding, event_cls), dim=1)  # event_num, 2*feature_size即2x768

        pair_embedding = self.node2pair(node_embedding)
        pair_embedding = pair_embedding.view(event_num * event_num, -1)  # 9x4608
    ...
    def predict(self, input_ids, attn_mask, pos_ids, event_mask, graph, distan, causal, logits=None):
        ...
        logits = logits + logits.transpose(0, 1)  #最后输出是event_num,event_num,2
        ...
        #使用torch.diag_embed和torch.diag函数来创建一个对角矩阵，其主对角线为1，其余为0。之后，使用异或操作^将logits_mask的对角线元素设为0，非对角线元素保持为1。
        pred = pred & logits_mask #使用logits_mask来确保pred和golden都只在对角线之外的位置有值（即只考虑非对角线元素）
        golden = golden & logits_mask
        right = (pred & golden) #计算pred和golden的交集，即同时满足预测和真实值的元素位置。
    ...
    def node2pair(self, node_embedding):
        ...
        pair_embedding = torch.cat((node_embedding,node_embedding.transpose(0, 1)),dim=2)  # event_num,event_num,feature_size 4x768 3072
