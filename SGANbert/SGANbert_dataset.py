...
import os
import time
from SGANbert_option import option
from tqdm import tqdm
import numpy as np
import torch
from torch.utils.data import Dataset, DataLoader
from SGANbert_graph_build import graph_builder
...
class PPATbert_dataset(Dataset):
    ...
    def getdata(self):
        ...
        for doc in tqdm(self.docs):
            ...
            # events
            events = doc['events']   #确保事件不能重复标记索引
            # if events == [] :
            #     docid -= 1
            #     continue
            tid2mid = {}
            ...
            graph = self.graph_builder.build_graph((distan==0).long(), self.max_graph_num, conjugate=self.conjugate)  #断点
            graph.add((distan==0).long(), 'intra')

            token_ids = []
            for tt in token_piece:
                token_ids.append(torch.tensor(self.tokenizer.convert_tokens_to_ids(tt), dtype=torch.long))  #将原始的分词结果转换为tensor随机张量
            token_ids = torch.stack(token_ids, dim=0)

            pos_piece = torch.tensor(pos_piece, dtype=torch.long)  #位置id变为tensor
    ...
    def sentence_tokenize(self, raw_tokens, raw_event, raw_sentence_id):
        ...
        for x, t in enumerate(raw_tokens):
            ...
            tokens += tt  #因为有的单词并不在词表里，所以一个raw_token会被拆分成多个
            token_idx += [raw_token_idx[x]] * len(tt)  #基于上述原因所以要乘上len（tt）
            sentence_id += [raw_sentence_id[x]] * len(tt)

        idx_map = []  #根据`token_idx`列表中的元素来分组
        for i in range(len(token_idx)):
            if i == 0 or token_idx[i] != token_idx[i-1]:
                idx_map.append([])
            idx_map[-1].append(i)

        events = []  #因为tokenizer分组之后，token_id的列表内容变多了，所以要更新events现在的索引值
        ...
        sentence_end = [len(sentence_id) - 1] #用来找出句子的结束位置
    ...
    def sentence_cut_piece(self, sentence_start, sentence_end, events):
        piece_contain_events = [] #句子的片段中包含的事件 0 1 2....
        sentence_piece = [] #句子的片段
    ...
    def get_mask(self,tokens,sentence_piece,piece_contain_events,events):
        def tid2pid(x):
            return x+1
        event_mask = torch.zeros((len(events),len(sentence_piece),self.maxlen), dtype=torch.long)#[events_num,sentence_piece,max_len]
        ...
        for i in range(len(sentence_piece)):
            ...
            token_pie += self.PAD * pad_len  #token的填充pad
            pos_pie += [0] * pad_len # 位置编码设置为0
