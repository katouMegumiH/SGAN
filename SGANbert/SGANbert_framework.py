...
from SGANbert_dataset import get_dataloader
from SGANbert_model import PPATbert_model
    ...
    def __eval(self, model, dataloader, error_analysis, mode='eval'):
        ...
        with torch.no_grad():
            for data_batch in dataloader:
                ...
                count.right_num += metric_score['right num'] #预测对的数量
                count.pred_num += metric_score['pred num'] #预测的总数量
                count.golden_num += metric_score['golden num'] #真实值的总数量

                count.inter_right += metric_score['inter right num']#distan不为0，也就是句子间
                count.inter_pred += metric_score['inter pred num']
                count.inter_golden += metric_score['inter golden num']

                count.intra_right += metric_score['intra right num']#distan为0，也就是句子内
